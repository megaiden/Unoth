...
public class DungeonGenerator : MonoBehaviour
{
    ...
    private void Start()
    {
        viewPlayer = GetComponent<PhotonView>();

        if (viewPlayer.Controller.IsMasterClient)
        {
            // DungeonStart
    ...
    private void CreateAllGameObjects()
    {
        ...
        for (int i = 0; i < dungeonSegmentsAmount; i++)
        {
            for(int j = 0; j < dungeon.segments[i].rooms; j++)
            {
                dungeonFragments[count].mainObject = PhotonNetwork.Instantiate(sitePrefab.name, dungeonFragments[count].position, quaternion.identity);
                dungeonFragments[count].mainObject.name = "Site " + count;
                dungeonFragments[count].mainObject.transform.position = dungeonFragments[count].position;
                dungeonFragments[count].mainObject.transform.parent = transform;
